...
class Nft_marketplace extends _i1.GeneratedContract {
  Nft_marketplace(
  ...
  /// like the gas price, nonce and max gas. The `data` and `to` fields will be
  /// set by the contract.
  Future<String> createMarketSale(BigInt tokenId,
      {required _i1.Credentials credentials,
      _i1.Transaction? transaction}) async {
    final function = self.abi.functions[3];
    assert(checkSignature(function, 'be9af536'));
  ...
  /// like the gas price, nonce and max gas. The `data` and `to` fields will be
  /// set by the contract.
  Future<String> createToken(String tokenURI, BigInt price,
      {required _i1.Credentials credentials,
      _i1.Transaction? transaction}) async {
    final function = self.abi.functions[4];
    assert(checkSignature(function, '72b3b620'));
    final params = [tokenURI, price];
    return write(credentials, transaction, function, params);
  }
  ...
  /// set, the function will be evaluated in the specified block. By default, the
  /// latest on-chain block will be used.
  Future<List<dynamic>> fetchItemsListed({_i1.BlockNum? atBlock}) async {
    final function = self.abi.functions[5];
    assert(checkSignature(function, '45f8fa80'));
    final params = [];
    final response = await read(function, params, atBlock);
  ...
  /// like the gas price, nonce and max gas. The `data` and `to` fields will be
  /// set by the contract.
  Future<String> resellToken(BigInt tokenId, BigInt price,
      {required _i1.Credentials credentials,
      _i1.Transaction? transaction}) async {
    final function = self.abi.functions[13];
    assert(checkSignature(function, 'e219fc75'));
    final params = [tokenId, price];
    return write(credentials, transaction, function, params);
  }
  ...
  /// like the gas price, nonce and max gas. The `data` and `to` fields will be
  /// set by the contract.
  Future<String> updateListingPrice(BigInt _listingPrice,
      {required _i1.Credentials credentials,
      _i1.Transaction? transaction}) async {
    final function = self.abi.functions[21];
    assert(checkSignature(function, 'ae677aa3'));
    final params = [_listingPrice];
    return write(credentials, transaction, function, params);
  }
